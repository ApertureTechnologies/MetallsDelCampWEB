    ...
    <footer className="bg-black text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Instalaciones */}
          <div>
            <h3 className="text-lg font-semibold mb-6">Direcciones de Instalaciones</h3>
              ...
              {installations.map((installation, index) => (
                <div key={index} className="flex items-start space-x-3">
                  ...
                  <div className="text-sm">
                    <p className="text-gray-300">{installation.address}</p>
            ...
            <div className="space-y-4">
              {contactInfo.map((contact, index) => (
                <div key={index} className="flex items-center space-x-3">
                  ...
                  <a 
                    href={contact.href}
                    className="text-sm text-gray-300 hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded"
                  >
                    {contact.label}
                  </a>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0">
            ...
            <div className="flex flex-wrap justify-center md:justify-start items-center space-x-6">
              {legalLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="text-sm text-gray-400 hover:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded"
                >
                  {link.name}
                </Link>
              ))}
            ...
            <div className="text-sm text-gray-400 text-center md:text-right">
              <p>Copyright © {currentYear} METALLS DEL CAMP</p>
            </div>
          </div>
        </div>
      </div>
